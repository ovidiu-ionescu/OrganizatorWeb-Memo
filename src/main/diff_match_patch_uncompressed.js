...
export const merge = (base, mine, remote) => {
  const dmp = new diff_match_patch();
  const diff = dmp.diff_main(base, mine, true);
  if (diff.length > 2) {
    dmp.diff_cleanupSemantic(diff);
  }
  const patch_list = dmp.patch_make(base, mine, diff);
  console.log('Patch list', patch_list)
  const patch_text = dmp.patch_toText(patch_list);
  console.log('Patch', patch_text)
  const patches = dmp.patch_fromText(patch_text);
  const results = dmp.patch_apply(patches, remote);
  return results[0];
} 